...
class Person(object):
    ...
    def add_child(self, child):
        """ Set the child of this person

         Args:
             child (:obj:`Person`): this person's child

         Raises:
             :obj:`PersonError`: if `child`'s gender is unknown
         """
        if child.gender == Gender.UNKNOWN:
            raise PersonError("child '{}''s gender is unknown".format(child.name))
        if self.gender == Gender.UNKNOWN:
            raise PersonError("person named '{}' cannot add child with unknown gender".format(self.name))
        if self.gender == Gender.MALE:
            Person.set_father(self=child, father=self)
        if self.gender == Gender.FEMALE:
            Person.set_mother(self=child, mother=self)

    def remove_father(self):
        """ Remove the father of this person

        Args:
            father (:obj:`Person`): the 'father' to be removed

        Raises:
            :obj:`PersonError`: if `person`'s father is None
            :obj:`PersonError`: if father doesn't have a child named 'person'
        """
        if self.father is None:
            raise PersonError("person named '{}''s father is unknown".format(self.name))
        elif self not in self.father.children:
            raise PersonError("person named '{}' doesn't have a child named '{}'".format(
                self.father.name, self.name))
        self.father.children.discard(self)
        self.father = None

    def remove_mother(self):
        """ Remove the mother of this person

        Args:
            mother (:obj:`Person`): the 'mother' to be removed

        Raises:
            :obj:`PersonError`: if `person`'s mother is None
            :obj:`PersonError`: if mother doesn't have a child named 'person'
        """
        if self.mother is None:
            raise PersonError("person named '{}''s mother is unknown".format(self.name))
        elif self not in self.mother.children:
            raise PersonError("person named '{}' doesn't have a child named '{}'".format(
                self.mother.name, self.name))
        self.mother.children.discard(self)
        self.mother = None
